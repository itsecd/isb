...
from .consts import KEY_COUNT_BITS_FOR_TRIPLEDES, INIT_VECTOR_COUNT_BYTE_FOR_TRIPLEDES, BLOCK_LEN_BITS_PADDING_FOR_TRIPLEDES, DEFAULT_KEY_BYTES_FOR_TRIPLEDES, DEFAULT_LEN_BYTES_PADDING_BLOCK_FOR_TRIPLEDES
    ...
    def generate_key(len_key: int = DEFAULT_KEY_BYTES_FOR_TRIPLEDES, 
        ...
        try:
            if not len_key * type_len.bits in KEY_COUNT_BITS_FOR_TRIPLEDES:
                raise ValueError(f"Count bits not value, your {len_key * type_len.bits} bits, need to {KEY_COUNT_BITS_FOR_TRIPLEDES}")

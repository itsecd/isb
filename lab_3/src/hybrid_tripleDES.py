...
def encrypt_text(text: bytes,
    ...
    try:
        ...
        original_symmetric_key = asymmetric.decrypt(symmetric_key,
                                                    asymetric_key)

        if len_block_padding:
            encrypted_text = SymmetricTripleDES.encrypt(text, 
...
def decrypt_cipher(cipher: bytes,
    ...
    try:
        ...
        if len_block_padding:
            decrypted_text = SymmetricTripleDES.decrypt(
                           cipher, 
                           original_symmetric_key, 
                           len_block_padding,
                           type_len_block_padding)

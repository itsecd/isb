...
def decrypt_text(enc_letters: list, text_for_decrypt: str, arr_decrypt_letters: dict) -> str:
    """
    Decrypt text using frequency analysis algorithm

    :param enc_letters: List of letters by frequency in the decrypted text
    ...
    dictionary = dict(zip(list(arr_decrypt_letters.keys()), enc_letters))
...
def write_result(enc_letters: list, path_decrypt: str, path_key: str, path_input: str) -> None:
    """
    ...
    try:
        ...
        keys = dict(zip(list(frequency(txt_reader(path_input))), enc_letters))
...
if __name__ == "__main__":
    encrypted_letters: list = json_reader("constants.json")["encrypted_letters"]

    paths = json_reader(json_reader("constants.json")["paths"])
    try:
        write_result(encrypted_letters, os.path.join(paths["folder"], paths["decrypt"]),
